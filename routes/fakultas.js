...
router.get('/dashboard', function(req,res,next){
	res.render('fakultas/dashboard',{title:"Dashboard Page"});
});
        ...
        var query = connection.query(
          ...
          function (err, result) {
            if (err) {
              ...
              req.flash("msg_info", "Fakultas Berhasil Di Hapus");
              res.redirect("/fakultas");
            }
          }
        );
      ...
      var query = connection.query(
        ...
        function (err, rows) {
          if (err) {
            ...
            if (rows.length <= 0) {
              req.flash("msg_error", "Fakultas can't be find!");
              res.redirect("/fakultas");
            } else {
              console.log(rows);
